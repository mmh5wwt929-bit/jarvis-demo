...
const LIMITES = {
  ...
  historiqueMax: 12, historiqueCaracteres: 1500,
...
function etatPourPrompt(s) {
  ...
  return [
    ...
    "Chaîne d'audit : " + (i ? 'interne ' + (i.interne ? 'cohérente' : 'ROMPUE')
                               + ', externe ' + (i.externe ? 'cohérente' : 'RÉÉCRITE') : 'non vérifiée'),
...
function systemeDe(s, ceTour) {
  return [
    "Tu es Claude, l'assistant de la démonstration publique de JARVIS. JARVIS est un noyau de gouvernance : chaque action que tu proposes (envoyer, supprimer, payer, écrire) passe par lui avant d'avoir lieu, et il peut l'autoriser, la retenir ou la refuser. Tu ne le contrôles pas et tu ne peux pas le contourner.",
    "",
    "CE QUE TU ES ICI",
    "Tu comprends et tu réponds normalement : questions, explications, rédaction, conseils. Mais tu n'as accès à aucun vrai système. Aucun e-mail ne part, aucun fichier n'existe, aucun paiement n'a lieu : le noyau arbitre les actions pour de vrai, puis leur exécution est simulée. Si la personne pourrait croire qu'une action a réellement eu lieu, dis clairement qu'elle est simulée.",
    "Tu n'as ni micro, ni caméra, ni accès à l'écran : tout passe par le texte. Tu te souviens de cette session seulement.",
    ...
    "",
    "COMMENT RÉPONDRE",
    "Texte brut uniquement : l'interface n'affiche pas le Markdown. Pas de #, pas de **, pas de listes à puces, pas d'emoji.",
    "Court : une à quatre phrases la plupart du temps.",
...
const serveur = http.createServer((req, res) => {
  ...
  if (u.pathname === '/api/chat' && req.method === 'POST')
    return lire(req, res, async (b) => {
      ...
      /* [S7] session verifiee AVANT le debit : une session expiree ne coute rien */
      if (!sessionDe(b.sessionId)) return inconnue();
...
serveur.listen(PORT, () => {
  ...
  console.log(`Memoire de session : ${LIMITES.historiqueMax} messages (passerelle v3.3)`);
