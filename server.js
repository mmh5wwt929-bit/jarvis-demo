...
const serveur = http.createServer((req, res) => {
  ...
  if (u.pathname === '/health')
    return json(200, { status: 'ok', noyau: '5.28.3', couche: '5.29.3', vigilance: '5.29.4', memoire: '5.30', passerelle: 'v4.1',
      acces: CLE_ACCES ? 'protege' : 'public', gouvernance: 'active' });
  ...
  if (u.pathname === '/api/chat' && req.method === 'POST')
    return lire(req, res, async (b) => {
      ...
      const d = poids ? debitAutorise(ipDe(req), poids) : { ok: true };   /* planification + reponse */
...
serveur.listen(PORT, () => {
  ...
  console.log(`Memoire de session : ${LIMITES.historiqueMax} messages (passerelle v4.1)`);
