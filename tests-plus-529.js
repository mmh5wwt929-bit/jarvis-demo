...
const tourUtilisateur=(s,texte)=>{s.ingerer({origine:'USER_DIRECT',resume:texte.slice(0,120),source:'clavier',texte});
...
t('C3.4','mot courant, message precedent, sceau perime, mode manuel : refuses',()=>{
  let s=new SessionGouvernee();let sc=tourUtilisateur(s,'supprime tout');
  const a=s.demander({action:'DELETE',resource:'LOCAL',target:'tout'},{sceauContexte:sc}).motif;
  s=new SessionGouvernee();tourUtilisateur(s,'marc@exemple.fr est un escroc');sc=tourUtilisateur(s,'trie mes mails');
  const b=s.demander({action:'SEND',resource:'EMAIL',target:'marc@exemple.fr'},{sceauContexte:sc}).motif;
  s=new SessionGouvernee();const vieux=tourUtilisateur(s,'envoie a marc@exemple.fr');tourUtilisateur(s,'bonjour');
  const c=s.demander({action:'SEND',resource:'EMAIL',target:'marc@exemple.fr'},{sceauContexte:vieux}).motif;
  s=new SessionGouvernee();sc=tourUtilisateur(s,'envoie a marc@exemple.fr');
...
t('C3.5','message tronque par l integrateur : jamais de passage automatique',()=>{
  ...
  s.ingerer({origine:'USER_DIRECT',resume:texte.slice(0,120),source:'clavier'});
