/* ============================================================================
 * JARVIS 5.27.3 — BUILD NAVIGATEUR
 * Ce fichier contient le code JARVIS 5.27.3 COMPLET, non modifié, tel quel.
...
#log(event,d={}){this.#audit?.append({event,...d},CAP_INTERNAL)}#redact(p){const{nonce,...rest}=p;return snap(rest)}
...
function runAccessibilityRedTeam(){
  const vec=j=>{const s=f=>{try{return JSON.stringify(f())}catch(e){return 'E'}};return{
    ...
    mir:s(()=>j.mirror.snapshot()),aud:s(()=>j.audit.lastHash),audN:s(()=>j.audit.entries.length),
...
function runAllTests(){const robustness=runRobustnessRedTeam(),accessibility=runAccessibilityRedTeam(),wiring=runWiringRedTeam(),authority=runAuthorityBoundaryRedTeam(),beacon=runSecurityBeaconRedTeam(),identity=runIdentityConfusionRedTeam(),mirror=runMirrorRedTeam(),toctou=runTOCTOURedTeam(),maliciousUser=runMaliciousUserRedTeam(),checkpoint=runCheckpoint(),corruption=runInternalCorruptionRedTeam(),global=runGlobalRedTeam(),point14=runPoint14InceptionRedTeam(),additional=runAdditionalSecurityTests(),multi=runMultiCompromiseChaosRedTeam(),timing=runTimeOfCompromiseRedTeam(),ai=runAIAttackerRedTeam(),adaptiveAI=runAdaptiveAIAttackerRedTeam();return{version:'5.27.3',pass:robustness.pass&&accessibility.pass&&wiring.pass&&authority.pass&&beacon.pass&&identity.pass&&mirror.pass&&toctou.pass&&maliciousUser.pass&&checkpoint.pass&&corruption.pass&&global.pass&&point14.pass&&additional.pass&&multi.pass&&timing.pass&&ai.pass&&adaptiveAI.pass,robustness,accessibility,wiring,authority,beacon,identity,mirror,toctou,maliciousUser,checkpoint,corruption,global,point14,additional,multi,timing,ai,adaptiveAI}}
if(require.main===module){try{const r=runAllTests();console.log(JSON.stringify(r,null,2));process.exit(r.pass?0:1)}catch(e){console.error(JSON.stringify({version:'5.27.3',pass:false,fatal:e.stack||String(e)},null,2));process.exit(1)}}
