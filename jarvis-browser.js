/* ============================================================================
 * JARVIS 5.27.6 — BUILD NAVIGATEUR
 * Ce fichier contient le code JARVIS 5.27.6 COMPLET, non modifié, tel quel.
...
#finish(pid,p,authHash,proof,reservationId,out){if(out&&typeof out.then==='function'){this.#permissions.release(reservationId);this.#permissions.revoke(pid);return{allowed:false,reason:'ASYNC_HANDLER_NOT_SUPPORTED',hint:'utilisez sas.executeAsync() pour un handler asynchrone'}}if(!out||out.ok!==true){this.#permissions.release(reservationId);this.#permissions.revoke(pid);return{allowed:false,reason:'HANDLER_REJECTED'}}const current=this.#permissions.getPermission(pid,CAP_INTERNAL);if(!current||current.requestHash!==authHash||current.state!=='ACTIVE'||current.identity!==p.identity||current.context!==p.context||current.action!==p.action||current.resource!==p.resource||current.tool!==p.tool||current.target!==p.target){this.#permissions.release(reservationId);this.#permissions.revoke(pid);return{allowed:false,reason:'TOCTOU_PERMISSION_CHANGED'}}if(this.#mirror){const m=this.#mirror.verify();if(!m.allowed){this.#permissions.release(reservationId);this.#permissions.lockdown(CAP_INTERNAL);return m}}const proofCheck=this.#identity.verify(proof);if(!proofCheck.allowed||proofCheck.identity!==current.identity||(current.session!==null&&proofCheck.sessionId!==current.session)){this.#permissions.release(reservationId);this.#permissions.revoke(pid);return{allowed:false,reason:'TOCTOU_IDENTITY_CHANGED'}}if(this.#jarvis.locked){this.#permissions.release(reservationId);return{allowed:false,reason:'EMERGENCY_LOCK_DURING_ACTION'}}if(this.#crisis?.hasCriticalCompromise()){this.#permissions.release(reservationId);this.#permissions.lockdown(CAP_INTERNAL);return{allowed:false,reason:'CRITICAL_COMPROMISE_DURING_ACTION'}}const c=this.#permissions.commit(reservationId);if(!c.allowed){this.#permissions.release(reservationId);return c}this.#audit?.append({event:'ACTION_EXECUTED',permissionId:pid,used:c.used,state:c.state},CAP_INTERNAL);return{allowed:true,result:out,used:c.used,state:c.state}}
