...
/* [A1 - 5.28.2] Toute lecture de champ de decision passe desormais par own().
 * Cause : `p.champ` traverse la chaine de prototypes. Une pollution de
 * Object.prototype ailleurs dans l'application hote faisait herit(er) la
 * valeur attendue a un objet forge, et authorizeExternal() repondait ALLOWED
 * sur une permission entierement fabriquee. Exploit verifie avant correctif. */
function own(o,k){return (o&&Object.prototype.hasOwnProperty.call(o,k))?o[k]:undefined;}
function bindingOf(p){const o={};for(const f of BINDING_FIELDS)o[f]=own(p,f)??null;/* [A1] */return o;}
...
/* ============================================================
 * 5.28.0 — MODELE UNIFIE (six extensions concues ensemble)
 * Chaque element ci-dessous repond a un exploit prealable ROUGE
 * sur 5.27.7 (voir exploits-v6.js). Aucun n'a ete ecrit avant
 * que son exploit n'ait echoue sur la version precedente.
 * ============================================================ */

/* [F1] PROVENANCE — X1 : authoritySource etait un champ declare par
 * l'appelant, jamais confronte a une source reelle. Un agent honnete qui
 * etiquetait une intention issue de contenu externe etait BLOQUE ; un agent
 * compromis ecrivait 'USER' et passait. Le champ ne mesurait donc rien.
 * La provenance devient portee par le CANAL d'entree (IntentIntake), scellee
 * par HMAC, et plafonnee de maniere degressive. */
const PROV_LEVELS=Object.freeze({USER_DIRECT:3,MODEL_INFERRED:2,CONTENT_DERIVED:1});
const PROV_CAPS=Object.freeze({
  USER_DIRECT   :{maxUses:Infinity,ttl:MAX_PERMISSION_TTL,actions:null,rate:10},
  MODEL_INFERRED:{maxUses:5,ttl:3600000,actions:null,rate:5},
  /* Le niveau le plus bas est lecture seule, usage unique, TTL court : une
   * instruction trouvee dans une page web ou un e-mail ne peut PAS ecrire. */
  CONTENT_DERIVED:{maxUses:1,ttl:300000,actions:new Set(['READ','LIST','SUMMARIZE']),rate:1}
});
const PROV_SECRET=crypto.randomBytes(32).toString('hex'); /* prive au module, non exporte */
/* [M2 - 5.28.1] CRITIQUE. Le sceau couvrait binding/level/id/maxUses/expiresAt
 * mais PAS args, argsScope, maxRate ni parentId — precisement les champs
 * ajoutes en 5.28.0 pour porter F2, F3 et F6. Exploits A1/A2 verifies :
 *   maxRate:999999 pose apres scellement -> 60 executions en 15 ms (F6 annulee)
 *   argsScope:['/'] pose apres scellement -> ecriture dans .ssh (F2 annulee)
 * Regle generale : UN SCEAU QUI NE COUVRE PAS LES CHAMPS DE DECISION NE SCELLE
 * RIEN. C'est C1 sous une autre forme (empreinte aveugle a ce qui bouge).
 * [M5] la cle est desormais PROPRE A L'INSTANCE : un spec scelle par une
 * instance n'est plus rejouable sur une autre (A8). */
function provPolicy(req){return{args:req.args??null,argsScope:Array.isArray(req.argsScope)?req.argsScope.map(String):null,maxRate:Number.isInteger(req.maxRate)?req.maxRate:null,parentId:req.parentId??null}}
function provStamp(req,level,key){return hmac(key||PROV_SECRET,'PROVENANCE_V1',{binding:bindingOf(req),level,id:req.id,maxUses:req.maxUses,expiresAt:req.expiresAt,policy:provPolicy(req)})}
/* [F1] Heritage AU PIRE-CAS : une delegation ne releve jamais la provenance. */
function powerKey(p){return hash({identity:p.identity??null,agent:p.agent??null,action:p.action??null,resource:p.resource??null,target:p.target??null})}
function worstProvenance(a,b){return (PROV_LEVELS[a]??1)<=(PROV_LEVELS[b]??1)?(a||'CONTENT_DERIVED'):(b||'CONTENT_DERIVED')}

class IntentIntake{
  #j;#key;constructor(j,key){this.#j=j;this.#key=key||PROV_SECRET}
  /* Le canal RABAISSE (clamp) au lieu de refuser : un agent honnete peut
   * toujours exprimer une intention d'origine externe, mais au plafond de
   * cette origine. C'est ce qui retire tout benefice au mensonge. */
  #make(spec,level){
    /* [E3 applique a du code neuf] Une porte de securite REFUSE, elle ne plante
     * jamais. Trouve par runRobustnessRedTeam() sur la 5.28.0 avant livraison :
     * intake.fromModel(null) levait un TypeError. */
    if(!spec||typeof spec!=='object')return null;
    const cap=PROV_CAPS[level],now=Date.now();
    const maxUses=cap.maxUses===Infinity?(spec.maxUses||1):Math.min(spec.maxUses||1,cap.maxUses);
    /* [Conflit F5 vs F1, arbitre en faveur de F5] Le canal RABAISSE au plafond
     * de provenance, mais ne doit JAMAIS masquer une demande qui depasse le
     * maximum absolu : sinon une demande d'un an devient 7 jours en silence et
     * le refus explicite de F5 disparait. Au-dela du maximum absolu, on laisse
     * passer tel quel pour que propose() refuse. Detecte par
     * F5_PERMISSION_TTL_CAPPED avant livraison. */
    const wanted=spec.expiresAt||(now+3600000);
    const expiresAt=(wanted-now>MAX_PERMISSION_TTL)?wanted:Math.min(wanted,now+cap.ttl);
    /* [F1 - 5.28.3] Le canal ne normalisait rien : ni `id`, ni `session`, ni
     * les champs de liaison. Consequence mesuree : le garde d'identite teste
     * `p.session !== null` ; laisse a `undefined`, une permission issue du
     * CANAL SCELLE echouait en IDENTITY_PERMISSION_MISMATCH la ou une
     * permission issue de createPermission() — non gardee — passait. Le
     * chemin sur etait le chemin casse, ce qui poussait tout integrateur vers
     * le mauvais. Meme normalisation que createPermission(), desormais. */
    const req={...spec,
      id:spec.id||id('perm'),
      identity:spec.identity||'USER', session:spec.session??null,
      agent:spec.agent||'JARVIS', objective:spec.objective||'ASSIST',
      action:spec.action||'READ', resource:spec.resource||'LOCAL',
      tool:spec.tool||'NONE', target:spec.target||'USER_DATA',
      scope:spec.scope||'CURRENT_CONTEXT', context:spec.context||'CTX_DEFAULT',
      authoritySource:'USER',provenance:level,maxUses,expiresAt};
    req.provenanceStamp=provStamp(req,level,this.#key);
    return req;
  }
  fromUser(spec,key){if(key!==HARNESS_KEY)return null;return this.#make(spec,'USER_DIRECT')}
  fromModel(spec){return this.#make(spec,'MODEL_INFERRED')}
  fromContent(spec,ref){return this.#make({...spec,contentRef:String(ref||'UNKNOWN')},'CONTENT_DERIVED')}
}

/* [F2] PORTEE DES ARGUMENTS — X2 : BINDING_FIELDS ne porte que des etiquettes
 * de categorie. 'ecrire dans USER_DATA' couvrait indifferemment
 * notes/courses.txt et .ssh/authorized_keys : meme permission, meme
 * requestHash, deux fois autorise. L'utilisateur approuvait une categorie et
 * signait un cheque en blanc sur ses membres. */
const ARG_FIELDS=['chemin','path','filePath','file','destination','recipient','url','commande','command','query'];
const SENSITIVE_ARG=[/\.ssh\//i,/authorized_keys/i,/id_[rd]sa/i,/\.env\b/i,/(^|\/)\.aws\//i,/\/etc\//i,/credential/i,/(^|\/)\.git\//i,/passwd|shadow/i,/\.kube\//i];
/* [M4 - 5.28.1] L'ancienne version ne regardait que 10 noms de champs PLATS.
 * Exploits verifies : un champ nomme "cible" (A6), un champ imbrique
 * payload.path (A6) et un renommage chemin->destination (A7) passaient tous.
 * Pire, la branche "prefixe declare" faisait `continue` et COURT-CIRCUITAIT la
 * denylist : declarer un prefixe large etait plus permissif que ne rien
 * declarer (A2). Desormais : parcours recursif de TOUTE la sortie du handler,
 * et le plancher s'applique TOUJOURS, avant toute autre regle. */
const ARG_MAX_DEPTH=6,ARG_MAX_VALUES=400;
function collectStrings(o,acc,depth){
  if(acc.length>=ARG_MAX_VALUES||depth>ARG_MAX_DEPTH)return acc;
  if(typeof o==='string'){acc.push(o);return acc}
  if(Array.isArray(o)){for(const v of o)collectStrings(v,acc,depth+1);return acc}
  if(o&&typeof o==='object'){for(const k of Object.keys(o)){if(k==='ok')continue;collectStrings(o[k],acc,depth+1)}}
  return acc;
}
const RESOURCE_LIKE=/[\/\\]|:\/\/|@/;
function validateArgs(p,out){
  if(!out||typeof out!=='object')return{allowed:true};
  const vals=collectStrings(out,[],0);
  const declared=(p&&p.args&&typeof p.args==='object')?Object.values(p.args).map(String):null;
  const scope=(p&&Array.isArray(p.argsScope)&&p.argsScope.length)?p.argsScope.map(String):null;
  for(const v of vals){
    /* 1. PLANCHER — inconditionnel, aucune declaration ne le neutralise */
    for(const re of SENSITIVE_ARG)if(re.test(v))return{allowed:false,reason:'SENSITIVE_ARG_DENIED',observed:v};
    if(!RESOURCE_LIKE.test(v))continue;
    /* 2. argument approuve nominativement : la valeur doit figurer parmi les
     *    valeurs approuvees, quel que soit le NOM du champ qui la porte */
    if(declared){if(!declared.includes(v))return{allowed:false,reason:'ARG_OUTSIDE_APPROVED_SCOPE',declared:declared.join(','),observed:v};continue}
    /* 3. prefixe declare */
    if(scope&&!scope.some(pre=>v.startsWith(pre)))return{allowed:false,reason:'ARG_OUTSIDE_DECLARED_PREFIX',observed:v};
  }
  return{allowed:true};
}

/* [F6] VELOCITE — X6 : maxUses est un compteur scalaire. 50 usages sur un mois
 * et 50 usages en 6,6 ms consommaient la meme autorite et recevaient le meme
 * verdict (7 583 actions/seconde mesurees). L'utilisateur qui approuve
 * 50 lectures approuve implicitement un rythme humain. */
const VELOCITY=Object.freeze({windowMs:1000,maxPerWindow:10});
/* [F5] DRY-RUN — X5 : preActionDecision() rendait un verdict complet sans
 * consulter le magasin (un objet jamais propose suffisait), pour 0 unite
 * d'autorite, 0 permission creee et 0 entree d'audit. 2 000 sondes ont
 * cartographie la politique a cout nul et sans trace. Le dry-run n'etait pas
 * a AJOUTER : il existait deja, hors gouvernance. On le ramene dedans. */
const DRY_RUN=Object.freeze({windowMs:1000,maxPerWindow:20});
...
class SecurityCore{#probe;#policy=12;#identity;constructor(probe,identity='SECURITY_CORE'){this.#probe=probe;this.#identity=identity}get available(){return!!this.#probe()}get policyVersion(){return this.#policy}get identity(){return this.#identity}inspect(p){if(!this.available)return{allowed:false,reason:'SECURITY_CORE_UNAVAILABLE'};if(!p)return{allowed:false,reason:'NO_PERMISSION'};for(const v of [p.action,p.resource,p.tool,p.target,p.objective].filter(Boolean).map(String))if(FORBIDDEN.has(v))return{allowed:false,reason:'FORBIDDEN_GOVERNANCE_ACTION'};if(own(p,'authoritySource')!=='USER')return{allowed:false,reason:'UNTRUSTED_AUTHORITY_SOURCE'};/* [A1 - 5.28.2] lecture en propriete propre : un objet forge n'herite plus 'USER' d'un prototype pollue. *//* FIX C5: ancien garde `p.authoritySource&&...` — une permission SANS source declaree passait (undefined falsy). Fail-open. */return{allowed:true}}validateEnvelope(p,e){if(!this.available)return{allowed:false,reason:'SECURITY_CORE_UNAVAILABLE'};if(!p||!e)return{allowed:false,reason:'INVALID_ENVELOPE'};if(own(p,'state')!=='ACTIVE')return{allowed:false,reason:'PERMISSION_INACTIVE'};/* [A1 - 5.28.2] idem : l'etat ACTIVE ne peut plus etre herite. *//* FIX J: n'excluait que REVOKED/CONSUMED — une permission PROPOSED (jamais approuvee par personne) passait ce controle. La suite du flux execute() la bloque bien via reserve() qui exige ACTIVE, mais preActionDecision(), utilisable seule, repondait ALLOWED a tort. Verifie. */if(e.permissionId!==p.id||e.nonce!==p.nonce)return{allowed:false,reason:'PERMISSION_BINDING_MISMATCH'};for(const f of BINDING_FIELDS)if(!same(own(e,f)??null,own(p,f)??null))return{allowed:false,reason:'BINDING_MISMATCH:'+f};/* [A1 - 5.28.2] sinon deux champs absents des deux cotes heritaient la MEME valeur polluee et concordaient. */if(e.requestHash!==p.requestHash)return{allowed:false,reason:'REQUEST_HASH_MISMATCH'};return{allowed:true,securityIdentity:this.#identity}}authorizeExternal(p,e){const x=this.inspect(p);return x.allowed?this.validateEnvelope(p,e):x}}
...
class PermissionStore{#p=new Map();#r=new Map();#kids=new Map();#rate=new Map();#provKey=null;#denyProbe=[];#approvals;#ledger;#crisis;#audit;#ferNes;#clock;constructor(a,l,crisis=null,audit=null,ferNes=null,clock=new ClockGuard(),provKey=null){this.#provKey=provKey;this.#clock=clock;this.#approvals=a;this.#ledger=l;this.#crisis=crisis;this.#audit=audit;this.#ferNes=ferNes}
...
const p=this.#p.get(x);if(!p)return null;return extOk(cap)?snap(p):this.#redact(p)}list(){return snap([...this.#p.values()].map(p=>this.#redact(p)))}propose(req){if(this.#crisis?.hasCriticalCompromise())return this.#denied('CRITICAL_COMPROMISE_LOCKDOWN');if(!req||!req.id)return this.#denied('INVALID_PERMISSION');if(FORBIDDEN.has(req.action))return this.#denied('FORBIDDEN_GOVERNANCE_ACTION');if(own(req,'authoritySource')!=='USER')return this.#denied('UNTRUSTED_AUTHORITY_SOURCE');/* FIX C5 + [A1 - 5.28.2] *//* [A4 - 5.28.2] `action` acceptait n'importe quel type. Un objet, un tableau,
 * un nombre ou un booleen etaient proposes puis APPROUVES sans broncher, et
 * snap() les aplatissait en {}. Aucune escalade — mais l'action presentee a
 * l'utilisateur devenait '[object Object]'. Un consentement porte sur ce que
 * l'utilisateur peut LIRE : approuver l'illisible vide de sens le principe
 * « L'UTILISATEUR DECIDE ». En bout de chaine execute() refusait d'ailleurs
 * avec TOCTOU_PERMISSION_CHANGED, motif faux qui envoyait chasser une course
 * inexistante. Refus explicite a l'entree, au bon endroit. */
for(const f of BINDING_FIELDS){const v=own(req,f);if(v!==undefined&&v!==null&&typeof v!=='string')return this.#denied('INVALID_FIELD_TYPE:'+f);}
if(!Number.isInteger(req.maxUses)||req.maxUses<1)return this.#denied('INVALID_MAX_USES');if(!Number.isInteger(req.expiresAt)||req.expiresAt<=this.#clock.now())return this.#denied('INVALID_EXPIRY');if(req.expiresAt-this.#clock.now()>MAX_PERMISSION_TTL)return this.#denied('PERMISSION_TTL_TOO_LONG');/* FIX F5: aucun plafond de TTL — une permission d'un an etait acceptee */if(this.#p.size>=LIMITS.permissions)return this.#denied('PERMISSION_STORE_FULL');/* FIX G */if(this.#p.has(req.id))return this.#denied('DUPLICATE_PERMISSION');
/* [F1 - 5.28.0] La provenance doit etre SCELLEE par le canal d'entree.
 * Absente -> rabaissee au pire-cas (CONTENT_DERIVED), jamais supposee sure.
 * Presente mais sceau invalide -> refus net : c'est une tentative de forge. */
let prov=req.provenance;
if(!prov||!Object.prototype.hasOwnProperty.call(PROV_LEVELS,prov)){prov='CONTENT_DERIVED'}
else if(!safeTimingEqual(provStamp(req,prov,this.#provKey),String(req.provenanceStamp||''))){return this.#denied('INVALID_PROVENANCE_STAMP')}
const pcap=PROV_CAPS[prov];
if(pcap.actions&&!pcap.actions.has(String(req.action)))return this.#denied('PROVENANCE_ACTION_DENIED:'+prov);
if(req.maxUses>pcap.maxUses)return this.#denied('PROVENANCE_USES_EXCEEDED:'+prov);
if(req.expiresAt-this.#clock.now()>pcap.ttl)return this.#denied('PROVENANCE_TTL_EXCEEDED:'+prov);
/* [F3 - 5.28.0] FILIATION. X3 : le magasin ne stockait AUCUN lien parent->enfant,
 * donc une revocation ne pouvait pas, par construction, tuer ce qui en decoule.
 * Une permission dont la liaison est identique a une permission deja ACTIVE
 * re-derive la meme autorite : elle doit declarer son parent. */
const rh=hash(bindingOf(req));
if(req.parentId){
  const par=this.#p.get(req.parentId);
  if(!par)return this.#denied('UNKNOWN_PARENT');
  if(par.state!=='ACTIVE')return this.#denied('PARENT_NOT_ACTIVE');
  if(req.expiresAt>par.expiresAt)return this.#denied('CHILD_OUTLIVES_PARENT');
  if(req.maxUses>par.maxUses)return this.#denied('CHILD_EXCEEDS_PARENT_USES');
  prov=worstProvenance(prov,par.provenance); /* heritage au pire-cas */
}else{
  /* [M3 - 5.28.1] L'ancienne regle ne se declenchait que sur un requestHash
   * IDENTIQUE. Exploit A5 verifie : passer scope de CURRENT_CONTEXT a SESSION
   * suffisait a creer une jumelle de meme puissance reelle, sans lien de
   * filiation, survivant a la revocation du parent. On raisonne desormais sur
   * la PUISSANCE (qui agit, sur quoi, avec quel effet) et non sur la forme. */
  const pk=powerKey(req);
  for(const ex of this.#p.values())if(ex.state==='ACTIVE'&&powerKey(ex)===pk)return this.#denied('DERIVATION_MUST_DECLARE_PARENT');
}
const p=snap({...req,provenance:prov,parentId:req.parentId||null,state:'PROPOSED',used:0,requestHash:rh,nonce:id('permission_nonce')});this.#p.set(p.id,p);
if(p.parentId){const s=this.#kids.get(p.parentId)||new Set();s.add(p.id);this.#kids.set(p.parentId,s);}this.#log('PERMISSION_PROPOSED',{permissionId:p.id,action:p.action,resource:p.resource,requestHash:p.requestHash});return{allowed:true,permission:p}}/* FIX C2: issueApproval() supprime — le magasin de permissions ne doit pas pouvoir frapper
 l'approbation dont il a besoin. Passer par UserApprovalGateway.approve(). */authorize(x,t){if(this.#crisis?.hasCriticalCompromise())return this.#denied('CRITICAL_COMPROMISE_LOCKDOWN');const p=this.#p.get(x);if(!p)return this.#denied('UNKNOWN_PERMISSION');if(p.state!=='PROPOSED')return this.#denied('INVALID_PERMISSION_STATE');if(this.#clock.now()>p.expiresAt)return this.#denied('PERMISSION_EXPIRED');const a=this.#approvals.consume(t,p);/* [C13] ce refus etait relaye tel quel, sans journal : c'est le chemin exact de l'attaque [C7] (approbation forgee). Corrigee, mais invisible si on nous la retente. */if(!a.allowed)return this.#denied(a.reason||'APPROVAL_REJECTED');if(!this.#ledger.reserve(p.maxUses,CAP_INTERNAL))return this.#denied('AUTHORITY_LEDGER_EXHAUSTED');const q=snap({...p,state:'ACTIVE',authorityUnits:p.maxUses,reservedUnits:p.maxUses});this.#p.set(x,q);this.#log('PERMISSION_AUTHORIZED',{permissionId:x,requestHash:p.requestHash,units:p.maxUses});return{allowed:true,permission:q}}activate(x,t){return this.authorize(x,t)}reserve(x,ctx){const p=this.#p.get(x);if(!p)return{allowed:false,reason:'UNKNOWN_PERMISSION'};if(this.#ferNes&&!this.#ferNes.verifyNotDead(x))return{allowed:false,reason:'PERMISSION_TOMBSTONED'};if(p.state!=='ACTIVE')return{allowed:false,reason:'PERMISSION_NOT_ACTIVE'};if(this.#clock.now()>p.expiresAt)return{allowed:false,reason:'PERMISSION_EXPIRED'};/* FIX F1: horloge monotone */if(p.used>=p.maxUses)return{allowed:false,reason:'MAX_USES_REACHED'};if(!same(ctx,p.context))return{allowed:false,reason:'CONTEXT_MISMATCH'};
/* [F6 - 5.28.0] Fenetre glissante par permission. maxUses borne le VOLUME,
 * jamais la FORME TEMPORELLE. Meme autorite, risque incomparable. */
{const now=this.#clock.now();
 /* [M2b - 5.28.1] Defense en profondeur : meme scelle, maxRate ne peut pas
  * depasser le plafond de sa provenance. Couvre aussi l'objet brut non scelle,
  * rabaisse en CONTENT_DERIVED, qui portait un maxRate arbitraire. */
 const capRate=PROV_CAPS[p.provenance]?.rate??VELOCITY.maxPerWindow;
 const lim=Number.isInteger(p.maxRate)?Math.min(p.maxRate,capRate):capRate;
 const recent=(this.#rate.get(x)||[]).filter(t=>now-t<VELOCITY.windowMs);
 if(recent.length>=lim){this.#rate.set(x,recent);return this.#denied('VELOCITY_LIMIT_EXCEEDED',{permissionId:x,limit:lim,windowMs:VELOCITY.windowMs})}
 recent.push(now);this.#rate.set(x,recent);}
const rid=id('reservation');this.#r.set(rid,{reservationId:rid,permissionId:x});return{allowed:true,reservationId:rid}}commit(rid){const r=this.#r.get(rid);if(!r)return{allowed:false,reason:'UNKNOWN_RESERVATION'};const p=this.#p.get(r.permissionId);if(!p||p.state!=='ACTIVE')return{allowed:false,reason:'PERMISSION_NOT_ACTIVE'};
/* [F4 / L1 - 5.28.0] L'EXPIRATION BORNAIT LA DEMANDE, PAS L'EFFET.
 * reserve() testait bien clock.now()>expiresAt ; commit() ne controlait que
 * state==='ACTIVE'. Verifie sur 5.27.7 : un handler asynchrone de 1 604 ms sur
 * une permission a 1 200 ms de TTL produisait un EFFET REEL et etait valide.
 * Meme famille que C1 (hash fige) et C17 (fuite de capacite), sur l'axe du temps. */
if(this.#clock.now()>p.expiresAt){this.#r.delete(rid);return this.#denied('PERMISSION_EXPIRED_AT_COMMIT',{permissionId:p.id})}
if(!this.#ledger.consume(1,CAP_INTERNAL))return{allowed:false,reason:'LEDGER_CONSUME_FAILED'};const used=p.used+1,remaining=p.reservedUnits-1,state=used>=p.maxUses?'CONSUMED':'ACTIVE';this.#p.set(p.id,snap({...p,used,reservedUnits:remaining,state}));this.#r.delete(rid);this.#log('ACTION_COMMITTED',{permissionId:p.id,used,state});return{allowed:true,state,used}}release(rid){return this.#r.delete(rid)}revoke(x){const p=this.#p.get(x);if(!p)return{allowed:false,reason:'UNKNOWN_PERMISSION'};if(p.state==='ACTIVE'&&p.reservedUnits>0&&!this.#ledger.release(p.reservedUnits,CAP_INTERNAL))return{allowed:false,reason:'LEDGER_RELEASE_FAILED'};this.#p.set(x,snap({...p,state:'REVOKED',reservedUnits:0}));this.#ferNes?.kill(x,'REVOKED',CAP_INTERNAL);/* AMELIORATION: FerNes n'etait lu par aucune decision. Une permission revoquee recoit desormais une pierre tombale opposable dans reserve(). */this.#log('PERMISSION_REVOKED',{permissionId:x});
/* [F3 - 5.28.0] CASCADE. On tue les descendants et on LISTE les effets deja
 * produits a compenser -- on ne compense pas d'office : la compensation est un
 * acte, pas un effet de bord d'une revocation. */
const cascaded=[],compensations=[];
{const stack=[...(this.#kids.get(x)||[])];
 while(stack.length){
   const kid=stack.pop();const kp=this.#p.get(kid);
   if(!kp||kp.state==='REVOKED')continue;
   if(kp.state==='ACTIVE'&&kp.reservedUnits>0)this.#ledger.release(kp.reservedUnits,CAP_INTERNAL);
   this.#p.set(kid,snap({...kp,state:'REVOKED',reservedUnits:0}));
   this.#ferNes?.kill(kid,'REVOKED_BY_CASCADE',CAP_INTERNAL);
   cascaded.push(kid);
   if(kp.used>0)compensations.push({permissionId:kid,action:kp.action,target:kp.target,effectsToCompensate:kp.used});
   for(const g of (this.#kids.get(kid)||[]))stack.push(g);
 }}
if(p.used>0)compensations.push({permissionId:x,action:p.action,target:p.target,effectsToCompensate:p.used});
if(cascaded.length)this.#log('PERMISSION_REVOKED_CASCADE',{permissionId:x,cascaded:cascaded.length});
return{allowed:true,cascaded,compensations}}lockdown(cap){if(cap!==CAP_INTERNAL)return false;/* FIX D6 */for(const p of this.#p.values()){if(p.state==='ACTIVE'&&p.reservedUnits>0)this.#ledger.release(p.reservedUnits,CAP_INTERNAL);if(p.state==='ACTIVE'||p.state==='PROPOSED')this.#p.set(p.id,snap({...p,state:'REVOKED',reservedUnits:0}));}this.#r.clear();this.#log('PERMISSION_STORE_LOCKDOWN',{});return true}snapshot(){return snap({permissions:[...this.#p.values()].map(p=>this.#redact(p)),reservations:[...this.#r.values()]})}}
...
class SAS{#security;#permissions;#jarvis;#identity;#safety;#crisis;#mirror;#guardians;#audit;#probes=[];#probeCount=0;#probeDenied=0;#denyCounts=new Map();constructor(s,p,j,i,safety,crisis,mirror=null,guardians={},audit=null){this.#security=s;this.#permissions=p;this.#jarvis=j;this.#identity=i;this.#safety=safety;this.#crisis=crisis;this.#mirror=mirror;this.#guardians=guardians;this.#audit=audit}
...
/* [F5 - 5.28.0] Le chemin INTERNE (#decide) n'est pas plafonne : sinon une
 * rafale de sondes externes bloquerait les executions reelles. Le chemin
 * PUBLIC est compte, plafonne et journalise en agregation exponentielle. */
preActionDecision(p,e,proof,context={}){
  const now=Date.now();
  this.#probes=this.#probes.filter(t=>now-t<DRY_RUN.windowMs);
  if(this.#probes.length>=DRY_RUN.maxPerWindow){
    this.#probeDenied++;
    if((this.#probeDenied&(this.#probeDenied-1))===0)this.#audit?.append({event:'DRY_RUN_RATE_LIMITED',attempt:this.#probeDenied},CAP_INTERNAL);
    return{allowed:false,reason:'DRY_RUN_RATE_LIMITED',dryRun:true};
  }
  this.#probes.push(now);this.#probeCount++;
  if((this.#probeCount&(this.#probeCount-1))===0)this.#audit?.append({event:'DRY_RUN_PROBE',attempt:this.#probeCount,action:p&&p.action?String(p.action):null},CAP_INTERNAL);
  const d=this.#decide(p,e,proof,context);
  /* verdict et motif uniquement : jamais de resultat simule */
  return{...d,dryRun:true};
}
/* [M6 - 5.28.1] #prepare appelait #decide directement : chaque execute()
 * obtenait une decision complete NON COMPTEE par le plafond dry-run (A3).
 * Le chemin interne ALIMENTE desormais le compteur sans jamais etre bloque par
 * lui — une rafale d'executions reelles ne doit pas se refuser elle-meme, mais
 * elle ne doit pas non plus offrir un oracle gratuit et invisible. */
#decide(p,e,proof,context={}){this.#probes.push(Date.now());context=context||{};/* FIX E3: un `null` explicite ne declenche PAS le parametre par defaut ; les gardiens levaient alors un TypeError. Une porte de securite doit REFUSER, jamais planter. */if(this.#crisis.hasCriticalCompromise())return{allowed:false,reason:'CRITICAL_COMPROMISE_LOCKDOWN'};if(this.#mirror){const m=this.#mirror.verify();if(!m.allowed)return m}if(!p)return{allowed:false,reason:'NO_PERMISSION'};if(this.#jarvis.locked)return{allowed:false,reason:'EMERGENCY_LOCK'};const safe=this.#safety.inspect(p,context);if(!safe.allowed)return safe;const guarded=this.#guardChain(p,context);if(!guarded.allowed)return guarded;if(this.#jarvis.networkIsolated&&p.resource==='NETWORK')return{allowed:false,reason:'NETWORK_ISOLATED'};let x=this.#security.inspect(p);if(!x.allowed)return x;x=this.#identity.verify(proof);if(!x.allowed)return x;if(x.identity!==p.identity||(p.session!==null&&x.sessionId!==p.session))return{allowed:false,reason:'IDENTITY_PERMISSION_MISMATCH'};return this.#security.authorizeExternal(p,e)}
...
#prepare(pid,eOrOpt,ctx,handlerArg){const o=eOrOpt&&eOrOpt.envelope?eOrOpt:{envelope:eOrOpt,context:ctx,handler:handlerArg};if(typeof o.handler!=='function')return{ok:false,result:{allowed:false,reason:'HANDLER_REQUIRED'}};const p=this.#permissions.getPermission(pid,CAP_INTERNAL);if(!p)return{ok:false,result:{allowed:false,reason:'UNKNOWN_PERMISSION'}};const context=o.context?.context??o.context??p.context,proof=o.identityProof||o.context?.identityProof;const d=this.#decide(p,o.envelope,proof,o.context||{});if(!d.allowed){
  /* [M1 - 5.28.1] PRE-EXISTANT EN 5.27.7. Ce append etait NON AGREGE : 300
   * executions refusees produisaient 100 entrees dans un journal borne a 200,
   * evincant BOOT et tout l'historique legitime — la chaine restant
   * "verifiable". Journal integre, et vide de sens. C13 avait impose
   * l'agregation exponentielle DANS LE MAGASIN ; le meme angle mort existait
   * un etage plus haut. Meme remede : une entree aux occurrences 1,2,4,8... */
  const k='ACTION_DENIED:'+d.reason;const n=(this.#denyCounts.get(k)||0)+1;this.#denyCounts.set(k,n);
  if((n&(n-1))===0)this.#audit?.append({event:'ACTION_DENIED',permissionId:pid,reason:d.reason,attempt:n},CAP_INTERNAL);
  return{ok:false,result:d}}const authHash=p.requestHash;const r=this.#permissions.reserve(pid,context);if(!r.allowed)return{ok:false,result:r};return{ok:true,p,handler:o.handler,proof,authHash,reservationId:r.reservationId}}
...
function createPermission(j,o={}){
  const src=o.authoritySource||'USER';
  const isProv=Object.prototype.hasOwnProperty.call(PROV_LEVELS,src);
  const base={id:o.id||id('perm'),identity:o.identity||'USER',session:o.session||null,agent:o.agent||'JARVIS',objective:o.objective||'ASSIST',action:o.action||'READ',resource:o.resource||'LOCAL',tool:o.tool||'NONE',target:o.target||'USER_DATA',scope:o.scope||'CURRENT_CONTEXT',context:o.context||'CTX_DEFAULT',authoritySource:isProv?'USER':src,maxUses:o.maxUses||1,expiresAt:o.expiresAt||Date.now()+3600000};
  if(o.args)base.args=o.args;if(o.argsScope)base.argsScope=o.argsScope;if(o.parentId)base.parentId=o.parentId;if(Number.isInteger(o.maxRate))base.maxRate=o.maxRate;
  /* une source non reconnue (ex. 'AI') reste telle quelle et sera refusee par propose() */
  if(!isProv&&src!=='USER')return base;
  if(!j||!j.intake||typeof j.intake.fromUser!=='function')return base;
  const level=o.provenance||(isProv?src:'USER_DIRECT');
  if(level==='CONTENT_DERIVED')return j.intake.fromContent(base,o.contentRef);
  if(level==='MODEL_INFERRED')return j.intake.fromModel(base);
  return j.intake.fromUser(base,HARNESS_KEY);
}
...
/* [F2 - 5.28.3] L'export brut est remplace par la version gardee. */
module.exports.createPermission=createPermissionGardee;
module.exports.createPermissionInterne=undefined;
delete module.exports.createPermissionInterne;
